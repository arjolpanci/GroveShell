...
pub(crate) struct AppState {
    ...
    /// The persistent floating desktop docks — one per monitor, present
    /// only when `dock_mode` is `"always"` or `"autohide"` (see
    /// `desktop_dock`). Empty in the default `"overview"` mode.
    pub(crate) docks: Vec<super::desktop_dock::DockWindow>,
}
...
pub(crate) fn role_of(hwnd: HWND) -> Role {
    STATE.with(|s| {
        ...
        if hwnd == st.calendar_hwnd {
            ...
            Role::QuickSettings
        } else if st.docks.iter().any(|d| d.hwnd == hwnd) {
            Role::Dock
        } else {
            Role::Other
        }
